...
> O objetivo da [Inferência Estatística]{.blue} é produzir afirmações sobre dada característica da população, na qual estamos interessados, a partir de informações colhidas de uma parte dessa população.

\

> [*População*]{.blue} é o conjunto de todos os elementos ou resultados sob investigação. [*Amostra*]{.blue} é qualquer subconjunto da população.

## Teste de Hipótese

\

> Uma afirmação sobre um parâmetro da população, [desejamos saber se os resultados experimentais provenienetes de uma amostra contrariam ou não tal afirmação]{.mark}.

\

*Uma afirmação que poderia ser feita é que a tarifa média é maior do que $30*

## Teste de Hipótese

\

Estruturando as hipóteses

$$H_0: \mu \geq 30$$
$$H_1: \mu < 30$$

Estamos sujeitos a cometer dois tipos de erro:

- Erro de tipo I: rejeitar $H_0$ quando essa é verdadeira. Chamamos de $\alpha$ a probabilidade de cometer esse erro 
- Erro de tipo II: não rejeitar $H_0$ quando $H_0$ é falsa. A probabilidade de cometer esse erro é denotada por $\beta$

*O erro tipo I nos diz sobre o risco que assumimos ao rejeitar a hipótese nula*

## Teste de Hipótese

\

### P-valor ou probabilidade de significância

\

$$
P(\bar{X} > \bar{x}_o | H_0\,\text{verdadeiro}) = P\left(\frac{\bar{X}-\mu}{\sigma \over \sqrt{n}} > \frac{\bar{x}_o-\mu}{\sigma \over \sqrt{n}}\right)
$$

```{r}
set.seed(456)
(fare <- titanic::titanic_test$Fare |> sample(50))
fare |> mean()
```
...
$$
P\left(\frac{\bar{X}-\mu}{\sigma \over \sqrt{n}} > \frac{\bar{x}_o-\mu}{\sigma \over \sqrt{n}}\right) = P\left(Z > \frac{37.4 - 30}{49.7 \over \sqrt{50}}\right) = P(Z > 1.05)
$$
...
Não há evidências estatísticas para rejeitar $H_0$.
...
library(ggplot2)
ggplot(
    data = data.frame(x = c(0, 60)), aes(x)
    ) +
  stat_function(fun = dnorm, n = 100, 
  args = list(mean = 30, sd = 49.7/sqrt(50))) + 
  ylab("") +
...
### Test t para 1 amostra

```{r}
t.test(x = fare, alternative = "greater", mu = 30)
```
...
::: {.columns}
::: {.column}
### Test t para 2 amostras

```{r}
dataset <- 
    titanic::titanic_train |>
    dplyr::select(Fare, Survived)

t.test(Fare ~ Survived, data = dataset)
```
:::
::: {.column}
```{r}
dataset |> 
    dplyr::filter(Fare < 300) |>
    ggplot2::ggplot() +
    ggplot2::aes(as.factor(Survived), Fare) +
    ggplot2::geom_boxplot() +
    ggplot2::theme_minimal() +
    ggplot2::theme(text = element_text(
        size = 25, 
        family = "IBM Plex Sans"))
```
:::
...
Teste [não-paramétrico]{.blue} para duas amostras

```{r}
wilcox.test(Fare ~ Survived, data = dataset)
```
...
```{r}
shapiro.test(titanic::titanic_test$Fare)
```
...
::: {.columns}
::: {.column}
> A [regressão logística]{.blue} é uma técnica estatística que tem como objetivo produzir, [a partir de um conjunto de observações, um modelo que permita a predição de valores tomados por uma variável categórica, frequentemente binária]{.mark}, a partir de uma série de variáveis explicativas contínuas e/ou binárias.
:::
::: {.column}
```{r}
titanic <-
...
## Regressão poisson e razão de prevalências

\

::: {.columns}
::: {.column width=30%}
A regressão de [Poisson]{.blue} é usada quando queremos projetar o valor de uma variável de resultado calculada à partir de [dados de contagem ou tabelas de contingências]{.mark}.
:::
::: {.column width=70%}
```{r}
...
dataset <-
    titanic |>
    dplyr::count(Survived, Sex, Pclass) |>
    dplyr::group_by(Sex, Pclass) |>
    dplyr::mutate(Total = sum(n)) |>
    dplyr::filter(Survived == "sim")

model <- glm(n ~ Sex + Pclass + offset(log(Total)),
    family = poisson,
    data = dataset
)
